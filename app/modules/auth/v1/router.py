...
async def login(request: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    if user.is_verified == False:
        return AuthResponse(status_code=401, access_token=None, token_type="None", message= "Email Has Not Been Verified")
    
    token = create_access_token(data={"sub": str(user.id)})
    return AuthResponse(status_code=200, access_token=token, token_type="bearer", message= "Token Issued")
...
async def logout(
    ...
    return {
        "success": True,
        "message": f"{current_user.email} logged out successfully"
    }
